...
app.directive('scInstructors', function() {
  return {
    ...
    template: '<div class="well sidebar-nav">' +
        ...
        '<div class="row" data-ng-repeat="instructor in instructorList">' +
        '<div class="col-md-6">{{instructor.name}}</div>' +
        '<div class="col-md-6"><div data-sc-follow-instructor data-instuctor-to-follow="instructor" /></div>' +
        '</div>' +
        '</div>',
...
app.directive('scFollowInstructor', function(/*scFollowedInstructors*/) {
  return {
    restrict: 'A',
    replace: true,
    template: '<div class="instructor-follow-button"><button data-ng-click="followInstructor()" class="btn btn-info btn-xs">Follow</button></div>',
